...
for N in dfm["N"].unique():
    if N in dfi.unique():
        ...
        df_moran["alpha_range"] = data_group.transform("max") - data_group.transform(
            "min"
        )
        df_moran["std_alpha"] = data_group.transform("std")
        df_moran = df_moran.groupby("UID").first()

        df_introspection = df_introspection.drop(
            columns=["process", "stochastic", "i_C"]
        )
        if "first_alpha" in df_introspection.columns:
            df_introspection.rename(
                columns={"first_alpha": "mutant_alpha"}, inplace=True
            )
        data_group = df_introspection.groupby("UID")["alpha_i"]
        df_introspection["alpha_range"] = data_group.transform(
            "max"
        ) - data_group.transform("min")
